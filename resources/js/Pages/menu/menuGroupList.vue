<template>
    <div class="px-4 py-6">
        ...
        <table class="min-w-full border-collapse rounded-md border border-gray-200">
            ...
            <tbody>
                <tr v-for="item in paginatedData" :key="item.menuGroupId" class="border-t hover:bg-gray-50">
        ...
        <TableFooter :total-items="props.menuGroups.length" :items-per-page="itemsPerPage" :current-page="currentPage"
            @current-page-change="updateCurrentPage" />
        <AddEdit v-if="showAddEdit" :show="showAddEdit" :obj="objMenuGroup" @close="closeAddEdit" />
        <Delete v-if="showDelete" :show="showDelete" :id="menuGroupId" @close="closeDelete" />
        <AssignMenu v-if="showMenuAssign" :show="showMenuAssign" :id="menuGroupId" :allMenu="allMenu"
